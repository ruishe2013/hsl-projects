 ...
 */
public class Level_First_Serial_New{
	
	private Log log = LogFactory.getLog(Level_First_Serial_New.class);
	...
	private Level_Second_Serial_New second_Level;
	...
	public void setSecond_Level(Level_Second_Serial_New second_Level) {
...
//			SerialPortQuartz serialPortQuartz = SerialPortQuartz.getInstance();
	...
	public void checkRunningPort(){
		...
		List<String> portList =  second_Level.getAllComPorts();
		if (portList.size() <= 0){
			// 没有发现任何串口
		}else{
	...
	public int testForTempHumi(){
		...
		for (EquipData equipData : commonDataUnit.getEquiMap().values()) {
			...
			if (openPort() == true){
				...
				rsFlag = writeReadSerial(address, equipmentId, Level_Second_Serial_New.READ_TEST_DATA, dataLen);
				// 3: 关闭串口
				closePort();
			}
			// 得到正确的帧,说明找到目标串口
			if (rsFlag  == second_Level.getPortService().getPortServiceValues().getSerial_right_Frame()){
				rsFlag = 1;
				break;
			}else{
				rsFlag = -1;
...
//				runFlag = 1;													// -- 测试用
			...
			// 通讯正常,跳出循环,不再重发
			if (runFlag == second_Level.getPortService().getPortServiceValues().getSerial_right_Frame()){
				break;
			}
			sendTimes --;
		}
		...
		while (sendTimes > 0){
			try {
				...
				if(openFlag){
					...
					rsBool = second_Level.getPortService().getPortServiceValues().getSerial_right_Frame() == 
						second_Level.readByte(-1, -1, Level_Second_Serial_New.READ_USEFULL_MAC, dataLen);//接收
				}
			} catch (Exception e) {
				//throw e;
				rsBool = false;
		...
		while (sendTimes > 0){
			try {
				...
				if(openFlag){
					...
					//接收
					rsBool = second_Level.getPortService().getPortServiceValues().getSerial_right_Frame() == 
						second_Level.readByte(-1, -1, Level_Second_Serial_New.READ_USEFULL_MAC, dataLen);
				}
			} catch (Exception e) {
				//throw e;
				rsBool = false;
